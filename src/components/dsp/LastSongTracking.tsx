  ...
  return (
    <div className="flex gap-3">
      {Array.from({ length: displayCount }).map((_, index) => {
        // If there's only one track, both buttons should be disabled except the first
        const isDisabled = !hasMultipleTracks && index > 0
...
export function LastSongTracking({ entidadId, selectedTrackIndex = 0 }: LastSongTrackingProps) {
  ...
  // Use the selected track index if it exists, otherwise fall back to the first track
  const safeIndex = selectedTrackIndex < data.length ? selectedTrackIndex : 0
  const selectedTrack = data[safeIndex]
