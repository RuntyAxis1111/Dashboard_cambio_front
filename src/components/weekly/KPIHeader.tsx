    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
      {kpis.map((kpi, index) => {
        ...
        return (
          <div
            key={index}
            className="bg-white border border-gray-300 rounded-xl p-6 hover:shadow-md transition-shadow"
          >
            <div className="text-sm font-medium text-gray-600 mb-2">{kpi.label}</div>
            <div className="text-2xl font-bold text-black mb-2">
              {kpi.value}
              {kpi.unit && <span className="text-base font-normal text-gray-500 ml-1">{kpi.unit}</span>}
            </div>
            {kpi.delta && (
              <div
